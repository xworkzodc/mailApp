...
//		return null;
	...
	public static void main(String[] args) {
		try {
			OkHttpClient client = new OkHttpClient();
			Request request = new Request.Builder().url("https://" + "us20" + ".api.mailchimp.com/3.0/lists").get()
					.addHeader("Content-Type", "application/x-www-form-urlencoded")
					.addHeader("User-Agent", "Mozilla/5.0")
					.addHeader("Authorization", "Basic 0da0d39d283f2a0d7113fadcd96e873e-us20")
					.addHeader("cache-control", "no-cache").build();
			Response response = client.newCall(request).execute();
			String result = response.body().string();
			...
			for (MailChimpMailDetails chimpMailDetails : chimpList.getLists()) {
				System.out.println("List is Founded....\n total member is {} and List id is {}"
						+ chimpMailDetails.getStats().getMemberCount() + chimpMailDetails.getId());
				System.out.println("Data is ------------->>>> {}" + chimpList);
				System.out.println(chimpMailDetails.getId());
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
