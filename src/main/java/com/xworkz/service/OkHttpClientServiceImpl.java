...
public class OkHttpClientServiceImpl implements OkHttpClientService {
	...
	public boolean sendCompaign(String compaignId, Integer integer) {
		...
		try {
			logger.info("Auth is {}", a);
			Response response = callClientApi(compaignId, encryptionHelper.decrypt(a), htp, "/actions/send");
...
//		return false;
	...
	private Response callClientApi(String compaignId, String a, String htp, String action) throws IOException {
		OkHttpClient client = new OkHttpClient();
		MediaType mediaType = MediaType.parse("application/json");
		RequestBody body = RequestBody.create(mediaType, "");
		Request request = new Request.Builder()
				.url("https://" + htp + ".api.mailchimp.com/3.0/campaigns/" + compaignId + action).post(body)
				.addHeader("Content-Type", "application/json").addHeader("Content-Type", "application/json")
				.addHeader("Authorization", encryptionHelper.decrypt(a)).build();
		Response response = client.newCall(request).execute();
		return response;
	}
...
//		return false;
		try {
			...
			if (finalHtmlString != null) {
				...
				htmlText.addProperty("html", finalHtmlString);
				logger.info("Html Json Is {}", htmlText.toString());
				Response response = callClientApi(campaignId, encryptionHelper.decrypt(a), htp, "/content");
				logger.info("Edit Response is {} ", response.body().string());
				if (response.isSuccessful())
					return true;
			}
	...
	public String createCompaign(String listId, SendMailDTO dto) {
		...
		try {
			...
			Request request = new Request.Builder().url("https://" + htp + ".api.mailchimp.com/3.0/campaigns")
					...
					.addHeader("Authorization", encryptionHelper.decrypt(a)).addHeader("cache-control", "no-cache")
...
//		return null;
	...
	public String getCompaignDetails() {
		try {
			...
			Request request = new Request.Builder().url("https://us20.api.mailchimp.com/3.0/campaigns").get()
					...
					.addHeader("User-Agent", "Mozilla/5.0").addHeader("Authorization", encryptionHelper.decrypt(auth))
	...
	public String getAllMailChimpList(Integer mailid) {
		...
		try {
			...
			Request request = new Request.Builder().url("https://" + htp + ".api.mailchimp.com/3.0/lists").get()
					.addHeader("Content-Type", "application/x-www-form-urlencoded")
					.addHeader("User-Agent", "Mozilla/5.0").addHeader("Authorization", encryptionHelper.decrypt(a))
					.addHeader("cache-control", "no-cache").build();
			Response response = client.newCall(request).execute();
	...
	public String getListIdFromListName(String listName, Integer mailid) {
		...
		logger.info("mail Id from dto {}", mailid);
	...
	/*
	 * public static void main(String[] args) { try { EncryptionHelper
	 * encryptionHelper = new EncryptionHelper(); String stEn= encryptionHelper.
	 * encrypt("Basic T20gQm46MGRhMGQzOWQyODNmMmEwZDcxMTNmYWRjZDk2ZTg3M2UtdXMyMA=="
	 * ); System.out.println("encrypted key= "+stEn);
	 * 
	 * String stDC= encryptionHelper.decrypt(stEn);
	 * System.out.println("decrypt key= "+stDC);
	 * 
	 * OkHttpClient client = new OkHttpClient(); Request request = new
	 * Request.Builder().url("https://" + "us20" +
	 * ".api.mailchimp.com/3.0/lists").get() .addHeader("Content-Type",
	 * "application/x-www-form-urlencoded") .addHeader("User-Agent", "Mozilla/5.0")
	 * .addHeader("Authorization", encryptionHelper.decrypt(stEn))
	 ...
	 * 
	 * MailChimpList chimpList = new Gson().fromJson(result, MailChimpList.class);
	 * for (MailChimpMailDetails chimpMailDetails : chimpList.getLists()) {
	 * System.out.
	 * println("List is Founded....\n total member is {} and List id is {}" +
	 * chimpMailDetails.getStats().getMemberCount() + chimpMailDetails.getId());
	 * System.out.println("Data is ------------->>>> {}" + chimpList);
	 * System.out.println(chimpMailDetails.getId()); } } catch (IOException e) {
	 * e.printStackTrace(); } }
	 */
