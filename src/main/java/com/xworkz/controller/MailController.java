...
public class MailController {
	...
	@RequestMapping(value="getListDetails.do",method=RequestMethod.GET,produces= {MediaType.APPLICATION_JSON_VALUE})
	@ResponseBody
	public MailChimpList getList(@RequestHeader("Accept") HttpHeaders headers, @RequestParam("msgType") Integer msgType) {
...
//		logger.info("Image Url Is {}",mailDTO.getImageURL());
	...
	public ModelAndView sendNewJoiner(@ModelAttribute SendMailDTO mailDTO) {
		...
		String file= clientService.getHTMLTextFromFile(mailDTO);
		return sendMail(mailDTO);
	}
	...
	private ModelAndView send(ModelAndView modelAndView, boolean result) {
		if(result) {
			...
			modelAndView.addObject("msg", "Mail Was Not Sended Please Try Again Letter................");
	...
	public String data() {
		...
		return xworkzService.getReport("", "");
	}
}